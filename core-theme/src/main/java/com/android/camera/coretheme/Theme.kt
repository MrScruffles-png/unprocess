...
fun UnprocessTheme(
    ...
    val canUseDynamicColor =
        Build.VERSION.SDK_INT >= Build.VERSION_CODES.S && !LocalInspectionMode.current

    // A camera viewfinder benefits from a consistently dark chrome. On Android 12+ the
    // accent roles come from the user's wallpaper, matching Android 16 / M3 Expressive while
    // preserving high contrast over the live preview. Explicit accessibility contrast settings
    // still take priority over wallpaper-derived colors.
    val colorScheme =
        when (contrast) {
            Contrast.MEDIUM -> mediumContrastDarkColorScheme
            Contrast.HIGH -> highContrastDarkColorScheme
            Contrast.DEFAULT -> {
                if (canUseDynamicColor) dynamicDarkColorScheme(context)
                else if (darkTheme) darkScheme else darkScheme
            }
        }
